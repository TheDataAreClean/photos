...
- add: `backfillExifLines()` in `local.js` — writes real EXIF values (camera/lens/aperture/etc, one per line) into a sidecar's still-blank `overrideExif`/`dateTaken` lines the first time the photo is processed, so a note created ahead of time in Obsidian (via the New Photo template, before the photo exists in the pipeline) ends up showing the real values instead of staying permanently blank. Idempotent, edits the raw text in place so comments/formatting aren't disturbed.
- add: `stripImageEmbeds()` in `local.js` — an Obsidian image embed (`![[photo.jpg]]`) left in the sidecar body is stripped before the body becomes the photo's `description`, so you can see the photo inline while captioning in Obsidian without the embed syntax leaking onto the live site.
...
- add: `scripts/backup-glass.js` — downloads the best-resolution copy of every Glass photo (visible + hidden, ~112) straight from Glass's CDN to `glass-backup/` and, if R2 is configured, to the bucket under a `glass/` prefix. The normal build pipeline never stored a full-resolution, unwatermarked copy of Glass originals anywhere — only a resized/watermarked copy in the uncommitted `dist/`. Run once, ahead of dropping Glass, so nothing is lost if the account or API access goes away. Also saves a manifest and the raw Glass API metadata. New export: `putObject()` in `r2.js`.
