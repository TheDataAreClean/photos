...
**Breaking**
- **Glass retired as a data source entirely — one unified `local.js` pipeline now, no more third-party CDN dependency.** `build/sources/glass.js` and five supporting scripts deleted; `config.js`'s `glass:` block, `deploy.yml`'s Glass CI steps/weekly cron/`GLASS_TOKEN`, and the "Glass" link button in the UI all removed. The 112 Glass originals were copied into `local/` and now flow through the same pipeline as everything else.
- **Every previously-Glass-sourced photo's URL changed**: `/photos/YYYY-MM-DD-glass-slug/` → `/photos/YYYY-MM-DD-slug/` (the `-glass-` infix dropped from IDs; local photo IDs unaffected). No redirects were set up for the old links.
- **Repo moved out of the Obsidian vault** to a plain directory (`~/Documents/thedataareclean/photos`) — Obsidian is no longer required to edit sidecars. `Templates/New Photo.md` replaced by root-level [`TEMPLATE.md`](TEMPLATE.md); README/APP docs reworded to drop Obsidian-specific framing (the underlying design — flat EXIF fields, standard Markdown embeds — was already editor-agnostic).

**Pipeline**
- Cloudflare R2 now backs up `local/` originals and is what CI builds from (`local/` is gitignored, so a fresh checkout has nothing without it). `npm run sync:r2` pushes new originals up; `downloadMissing()` pulls them into CI; both no-op silently without R2 env vars.
- `npm run publish:local`, also run every 20 min via a new launchd agent, backs up new originals to R2 then commits + pushes `sidecars/` — the push triggers the normal CI deploy.
- `npm run rename-photo -- <old-id> <new-id> --apply` — new, and now the only safe way to rename a live photo's slug (renames image + sidecar + any `series/*.md` reference together; renaming the sidecar alone silently orphans it).
- `glass-sidecars/` renamed to `sidecars/` — single home for every photo's caption/EXIF file regardless of source. EXIF fields are flat top-level frontmatter properties, not nested, and fall back live to the photo's real EXIF when left blank.
- EXIF auto-backfill: a sidecar's still-blank EXIF/`dateTaken` fields are filled with the real extracted values the first time a photo is processed (scoped strictly to frontmatter, after a bug let it overwrite caption text — see Fixes).
- `url.display`/`url.download` now AVIF instead of WebP — smaller at comparable quality; every photo re-encoded once on the first build after this change.
- `gps` dropped from the EXIF object — was being silently exported into public JSON with nothing reading it.

**Fixes**
- CI could silently deploy a zero-photo site if R2 was misconfigured — `downloadMissing()`'s failure was only ever logged as a warning. Root-caused (three distinct failure modes: wrong bucket, a token scoped for List but not Get, a rotated credential) and documented so it's a one-step diagnosis next time.
- `scripts/publish-local.js` could crash outright on a bad/placeholder R2 credential, aborting its git commit+push step too — now fails soft. Its auto-commit also used to run a bare `git commit`, which could sweep in unrelated staged changes — now scoped to `sidecars/`, plus a pre-flight YAML validity check and a rebase-before-push retry.
- `backfillExifLines()` matched its "still blank" regexes against the whole sidecar file, not just the frontmatter — a caption line like `lens:` could get silently overwritten with real EXIF data. Now scoped to inside the frontmatter delimiters only.
- Two orphaned/duplicate sidecars removed (`2026-03-19-after.md`, `2025-09-21-morning.md`); a stale "fall back to what Glass provides" template comment corrected to EXIF across every sidecar; the dead `glassAutoId:` field removed from the 5 sidecars that still had it.
- `scripts/rename-local.js` still read/wrote sidecars from the old `local/` path after the pipeline moved them to `sidecars/`, silently orphaning any sidecar it touched — now derives both paths from `config.js`.

**Tests**
- First unit test coverage (`test/`, Node's built-in test runner, no new dependency) for slug generation, sidecar parsing/overrides, and EXIF backfill — wired into CI as a required step before every build.
